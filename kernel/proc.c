...
found:
  ...
  //在allocproc函数里调用Step 2 创建的函数设置内核页表，
  //并且调用kvmmap函数将Step 3 设置的内核栈映射到页表k_pagetable里。
  //下面的代码是仿照procinit
...
你可参考kernel/vm.c中的freewalk，
其用于释放整个页表，但要求叶子页表的页项已经被清空。*/
static void
...
growproc(int n)
{
  ...
  if (n > 0)
  {
    ...
    // 添加复制函数
    myuvmcopy(p->pagetable, p->kernel_pagetable, sz - n, sz);
  }
  else if (n < 0)
  {
...
/*修改调度器（scheduler），
使得切换进程的时候切换内核页表。
参考方法：在进程切换的同时也要切换页表将其放入寄存器satp中，
请借鉴 kvminithart()的页表载入方式），
在调用 w_satp() 之后调用 sfence_vma()*/
//fixme
void scheduler(void)
{
  struct proc *p;
...
// void copy_from_user_to_kernel()
// {

// }

// fixme
//这一个复制函数感觉指导书上写的并不是很好
/*实际上vm中有一个函数是可以模仿的，就是uvmcpy
 */
void myuvmcopy(pagetable_t user, pagetable_t kernel, uint64 oldsz, uint64 newsz)
{
  if (newsz >= oldsz)
  {
